...
def save_user_data(user_id, name, age, gender, interests, about_me, username=None):
    ...
    conn = sqlite3.connect(DATABASE_NAME)
    cursor = conn.cursor()
    cursor.execute("INSERT OR REPLACE INTO users (user_id, name, age, gender, interests, about_me, username) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
def get_user_data(user_id):
    ...
    conn.close()
    if data:
        return {'name': data[0], 'age': data[1], 'gender': data[2], 'interests': data[3], 'about_me': data[4], 'username': data[5]}
    else:
        return None
...
def register_start(message):
    ...
    if registration_state:
        bot.reply_to(message, "Вы уже начали регистрацию. Пожалуйста, завершите ее.")
        return
...
def like_liker_callback(call):
    ...
    try:
        ...
        if liker_data:
            ...
            bot.send_message(user_id, f"Вы поставили лайк пользователю {liker_data['name']}. Его username: {username_text}")
...
def process_edit_name(message):
    ...
    try:
        ...
        bot.reply_to(message, 'Произошла ошибка. Пожалуйста, попробуйте еще раз /edit_profile')
...
def process_edit_age(message):
    ...
    try:
        ...
        if age < 0 or age > 150:
            bot.reply_to(message, "Пожалуйста, введите корректный возраст (от 0 до 150).")
            bot.register_next_step_handler(message, process_edit_age)
            return
...
def process_edit_about_me(message):
    ...
    try:
        ...
        if name is None or age is None or gender is None or interests is None:
            bot.reply_to(message, "Произошла ошибка. Не удалось получить данные. Попробуйте /edit_profile снова.")
        ...
        # После обновления профиля выводим основные кнопки
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        btn_search = types.KeyboardButton("Поиск")
...
def process_search_gender(message):
    ...
    try:
        ...
        if available_users:
            ...
            bot.send_message(message.chat.id, "К сожалению, больше пользователей с такими параметрами не найдено. Возвращаемся в главное меню.")
...
def show_user(message, user_id):
    ...
    if current_index < len(users):
        ...
        # Проверяем, есть ли взаимный мэтч
        if check_match(user_id, user['user_id']):
            username_text = f"@{user['username']}" if user.get('username') else "Нет username"
        else:
...
def process_name(message):
    ...
    try:
        ...
        bot.reply_to(message, 'Произошла ошибка. Пожалуйста, попробуйте еще раз /register')
...
def process_about_me(message):
    ...
    try:
        ...
        if name is None or age is None or gender is None or interests is None:
            bot.reply_to(message, "Произошла ошибка. Не удалось получить данные. Попробуйте /register снова.")
            clear_registration_state(user_id)
            return
        ...
        btn_edit_profile = types.KeyboardButton("Изменить профиль")  # Новая кнопка
        markup.add(btn_search, btn_likes, btn_edit_profile)
        bot.send_message(message.chat.id, "Спасибо за регистрацию! Теперь вы можете начать поиск", reply_markup=markup)
